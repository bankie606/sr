...
The goal of Sr is to provide a MapReduce-style framework for processing streams. Such a
...
Sr builds of previous work in the areas of MapReduce and Data Stream Management Systems.
...
called \emph{Data Stream Management Systems} (\emph{DSMSes}) are discussed. Sr is a type of DSMS that
...
processed. This is the data model that Sr uses.
...
\subsection{Spout}
Spouts are network-connected machines that generate data for an Sr cluster to process.
...
Sr achieves this configuration with a Jobfile. Jobfiles are instances of a special class
...
100ms of lost work is $\ll0.1\%$ of the total. Sr can simply treat these lost tasks as preemptively
...
The cluster I used to test Sr is a 5-node network consisting of~1 spout,~1 fetcher,~4 workers,~1
...
|-----------| \   |--------|
               \  |--------|
                - | worker |
                  |--------|
\end{verbatim}
\caption{Topology of the test Sr cluster used in the word count case study. Each box represents a single machine.}
...
The Sr source code is open source and available at:
\begin{center}
